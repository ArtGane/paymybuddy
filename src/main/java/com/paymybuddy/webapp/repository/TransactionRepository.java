...
public interface TransactionRepository extends JpaRepository<Transaction, Long> {
    ...
    @Query(value = "SELECT * FROM transaction t WHERE t.user_sender = :userSenderId ORDER BY t.creationDate DESC LIMIT :itemsPerPages OFFSET :offset", nativeQuery = true)
    ...
    @Query(value = "SELECT * FROM transaction t WHERE t.user_sender = :userSenderId ORDER BY t.creationDate DESC LIMIT :itemsPerPages", nativeQuery = true)
    List<Transaction> getUserTransactionsPage(@Param("userSenderId") Long userSenderId,
                                                    @Param("itemsPerPages") Integer itemsPerPages);

    @Query(value = "SELECT COUNT(*) FROM transaction t WHERE t.user_sender = :userSenderId", nativeQuery = true)
